...
const ContactCard = ({
  ...
  const handleClick = () => {
    if (type === "whatsapp" && number) {
      ...
      window.open(
        `https://wa.me/${cleanNumber}?text=Hello! I'm interested in your exhibition services.`,
        "_blank",
      );
    } else if (type === "wechat" && number) {
  ...
  return (
    ...
    >
      <button
        onClick={handleClick}
        className={`flex items-center gap-3 px-5 py-3 ${bgColor} ${hoverColor} text-white rounded-full shadow-lg transition-all duration-300 group cursor-pointer`}
      >
        <span className="shrink-0">{icon}</span>
        <div className="text-left">
          <p className="text-xs opacity-80">
            {type === "whatsapp" ? "WhatsApp" : "WeChat"}
          </p>
          <p className="text-sm font-semibold">{number || "Not set"}</p>
        </div>
      </button>
    </motion.div>
  );
...
export default function FloatingContact() {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (
        isOpen &&
        !(e.target as HTMLElement).closest(".floating-contact-container")
      ) {
        setIsOpen(false);
      }
    };
  ...
  // Don't show if no contact methods available
  if (!isVisible) return null;
  if (!contactInfo.whatsapp && !contactInfo.wechat && !contactInfo.primaryPhone)
    return null;
  ...
  return (
    <div className="floating-contact-container fixed bottom-6 right-6 z-50">
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            {/* WhatsApp Card - Only if number exists */}
            ...
            {/* WeChat Card - Only if ID exists */}
            {contactInfo.wechat && (
              <ContactCard
                type="wechat"
                number={contactInfo.wechat}
                icon={<WeChatIcon />}
                bgColor="bg-[#7BB32E]"
      ...
      <motion.button
        ...
        animate={{
          ...
          boxShadow: hasScrolled
            ? [
                "0 0 0 0 rgba(0, 153, 153, 0)",
                "0 0 0 15px rgba(0, 153, 153, 0)",
                "0 0 0 0 rgba(0, 153, 153, 0)",
              ]
            : "0 4px 20px rgba(0, 153, 153, 0.3)",
        }}
        transition={
          hasScrolled ? { duration: 2, repeat: 2, ease: "easeInOut" } : {}
        }
      ...
      >
        ...
        <div
          className={`relative flex items-center justify-center gap-2 ${isOpen ? "w-14 h-14 bg-gray-900" : "w-auto px-5 py-3.5 bg-linear-to-r from-primary to-primary-hover"} rounded-full shadow-2xl border border-white/20 transition-all duration-300`}
        >
          {isOpen ? (
            <CloseIcon />
          ) : (
